...
| oekakiURL     | 下書き機能(1)で使用する画像が配置されているURLパス名。URLパラメータ`oekaki_id`と組み合わせて使用する。詳細は[下書き機能](#下書き機能)の項(1)で解説。|  |
...
現在のDBバージョンは`3`で、以下のオブジェクトストアで構成されています。

|ストア名|内容|
|----|----|
|`save_manual`|セーブ／ロードのセーブデータ本体（最大50スロット）|
|`save_manual_meta`|セーブ／ロード画面の一覧表示用の情報（サムネイル・日時など。レイヤー画像を含まない）|
|`save_auto`|自動バックアップのセーブデータ（10ストローク毎に保存、最大20件）|
|`save_config`|ユーザー設定|
|`save_palette`|カラーパレット|

セーブデータ本体はレイヤーごとの画像データを含むため1件あたり数MBに達します。一覧表示のたびに全件を読み込むとスロット数に比例して重くなるため、表示に必要な情報のみを`save_manual_meta`に分離しています。

複数のタブでAXNOS Paintを開いている状態でDBのバージョン更新が必要になった場合、更新が行えないため警告を表示します。他のタブを閉じてからページを再読み込みしてください（※描画機能はそのまま使用できますが、セーブ／ロード機能などは利用できません）。

### 設定のインポート・エクスポート

[設定]-[バージョン情報]から、全ての設定項目とカラーパレットをまとめてjson形式のファイルに保存／復元できます。ブラウザや端末を変更する際の移行に使用できます。

設定値は単一のオブジェクトに集約されているため、設定項目を追加してもエクスポート処理へ追記する必要はありません。
ただし、復元した設定を各システムへ反映する処理（`AxpObj.applyConfigToRuntime()`）は個別に記述する必要があります。**値の反映に処理が必要な設定項目を追加した場合は、必ず同メソッドへ追記してください。** 追記を忘れると、インポートしても設定が反映されない（かつエラーも表示されない）状態になります。

特に注意が必要なのは、フォームの値そのものではなく、そこから計算して描画・設定する類の処理です。
プログラムから値を代入した場合は`input`／`change`イベントが発生しないため、イベントハンドラの中だけに反映処理を書くと、起動時とインポート時のどちらでも反映されません。
反映処理はイベントハンドラから呼び出せるメソッドとして実装し、`applyConfigToRuntime()`からも呼び出してください。
（例：筆圧カーブの描画`drawPressureCurve()`、ハライ/ハネの値のペンへの反映`applyFlickConfigToPen()`）
...
なお、旧バージョン（DBバージョン2以前）のデータが存在する場合は、起動時に一覧表示用ストア`save_manual_meta`が追加され、既存のセーブデータから表示用の情報が移送されます。セーブデータそのものは変更されません。
