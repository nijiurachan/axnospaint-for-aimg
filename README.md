...
| enableOdaiMode | 起動時にモード選択ダイアログ（お絵カキコ／お題絵）を表示します。お題絵モードでは、投稿時に絵から正方形を切り取り、お題枠（テキスト編集可）に合成して投稿します。`false`を指定すると常に通常モードで起動します。※下書き機能使用時はダイアログを表示せず通常モードで起動します。| true |
